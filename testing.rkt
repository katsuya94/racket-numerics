  ...
  (let ()
    (define (test* expect fun args kws kvs)
      ...
      (let ([res (if (procedure? fun)
       (let ([a-fun (a-lookup fun)])
         (if a-fun
           (a->r (if kws (keyword-apply fun kws kvs (map r->a args))
            (apply a-fun (map r->a args))))
           (if kws (keyword-apply fun kws kvs args)
             (apply fun args))))
       (car args))])
      (printf "~s\n" res)
...
(define (report-errs . final?)
  (let* ([final? (and (pair? final?) (car final?))]
    ...
    (parameterize ([current-output-port
                    (cond [(not ok?) (current-error-port)]
                          [final? (current-output-port)]
